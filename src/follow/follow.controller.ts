...
export class FollowController {
  ...
  async getSearchResult(
    ...
    try {
      const result = await this.followService.getSearchResult(
  ...
  async createFollow(
    ...
    try {
      const result = await this.followService.checkFollow(
        req.user.id,
        followingId,
      );
      if (!!result.deleted) {
      ...
      return new ResponseDto(ResponseCode.FOLLOW_FAIL, false, e.message, null);
    }
  }
}
