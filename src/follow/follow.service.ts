...
export class FollowService {
    ...
    try {
      ...
      const searchResult = await Promise.all(
        resultUsers.map(async (user) => {
          ...
          // 팔로우 여부
          followSearchDto.isFollowing = await this.userService.checkIfFollowing(
            userEntity,
            followSearchDto.id,
          );
          ...
          if (image == null) followSearchDto.image = null;
            ...
            followSearchDto.image = await this.s3Service.getImageUrl(
              userImageKey,
            );
          }
  ...
  async getFollowList(userId: number): Promise<FollowDto[]> {
    ...
    // 로그인한 사용자 = 팔로우하는 user
    const follows: UserFollowingEntity[] =
      await this.userService.getFollowingList(userId);

    // 팔로우 사용자들 정보 리스트
    const informs = await Promise.all(
  ...
  async getFollowerList(userId: number): Promise<FollowDto[]> {
    ...
    // 로그인한 사용자 = 팔로워
    const follows: UserFollowingEntity[] =
      await this.userService.getFollowerList(userId);
    ...
    const informs = await Promise.all(
      follows.map(async (follow) => {
        ...
        followDto.email = mateEntity.email;
        followDto.introduction = mateEntity.introduction;
        followDto.isFollowing = await this.userService.checkIfFollowing(
          user,
          mateEntity.id,
        );
  ...
  async checkFollow(
    ...
    try {
      ...
      if (!userEntity)
        throw new NotFoundException('요청을 보낸 사용자를 찾을 수 없습니다');
      const followingUser = await UserEntity.findOne({
        where: {
          id: followingId,
        },
      });
      if (followingUser.isQuit == true)
        throw new BadRequestException('탈퇴한 회원 입니다');
      if (!followingUser)
  ...
  async createFollow(
    ...
    try {
      const userEntity: UserEntity = await this.userService.findUserById(
        userId,
      );
  ...
  async deleteFollow(
    ...
    const followEntity: UserFollowingEntity =
      await UserFollowingEntity.findOneOrFail({
        where: { user: { id: userId }, followUser: { id: followingId } },
      });
