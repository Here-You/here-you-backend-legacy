...
export class SignatureController {
  ...
  async getMySignature(
    @Req() req: Request,
  ): Promise<ResponseDto<HomeSignatureDto[]>> {
  ...
  async patchSignatureLike(
    ...
    try {
      // [1] 이미 좋아요 했는지 확인
      const liked: SignatureLikeEntity =
        await this.signatureService.findIfAlreadyLiked(
          req.user.id,
          signatureId,
        );
      ...
      if (liked) {
        // 이미 좋아요했던 시그니처라면 좋아요 삭제
        result = await this.signatureService.deleteLikeOnSignature(
          liked,
          signatureId,
        );
        ...
        // 좋아요 한적 없으면 시그니처 좋아요 추가
        result = await this.signatureService.addLikeOnSignature(
          req.user.id,
          signatureId,
        );
        likeSignatureDto.liked = result.liked;
  ...
  async getSignatureDetail(
    ...
    try {
      ...
      const result = await this.signatureService.detailSignature(
        req.user.id,
        signatureId,
      );
      ...
      if (result.author.is_followed) {
        // 작성자가 본인이 아닌 경우
        if (result.author._id == null) {
          // 작성자가 탈퇴한 경우
          return new ResponseDto(
  ...
  async patchSignature(
    ...
    try {
      // 임시로 토큰이 아닌 유저 아이디 받도록 구현 -> 리펙토링 예정
      const result = await this.signatureService.patchSignature(
        signatureId,
        patchSignatureDto,
      );
  ...
  async getSignatureLikeList(
    ...
    try {
      const getLikeListDto: GetLikeListDto =
        await this.signatureService.getSignatureLikeList(
          req.user.id,
          signatureId,
        );
